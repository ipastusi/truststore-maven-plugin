        ...
        stage('Clone') {
            steps {
                sshagent(['github-creds']) {
                    git credentialsId: 'github-creds', url: 'git@github.com:automatictester/truststore-maven-plugin.git'
                }
            }
        }
        stage('Purge') {
            steps {
                sh 'rm -rf ~/.m2/repository/uk/co/automatictester/truststore-maven-plugin/'
            }
        }
...
//                    sh "./mvnw clean deploy -P release -Dgpg.passphrase=${GPGPP} -DskipTests -Dinvoker.skip=true"
                    echo 'not this time - already done - fixing problems with git permissions only'
                }
            }
        }
